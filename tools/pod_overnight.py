...
import numpy as _np
_gy, _gx = _np.mgrid[0:64, 0:64]
GW = _np.exp(-(((_gy-31.5)/18)**2 + ((_gx-31.5)/18)**2)/2).astype(_np.float32)
OUT = "/workspace/out"
...
def fetch_region(t, cov_target):
    ...
    def g(cy, cx):
        try:
            ...
            if len(b) == D * CH * CH:
                a = np.frombuffer(b, np.uint8).reshape(D, CH, CH)[27:27+DLAY]
...
def infer_map(model, torch, stack, stride):
    ...
    cnt = np.zeros((Hq, Wq), np.float32) + 1e-6
...
def main():
    ...
    log("remapping candidate segments with TUNED eyes (stride 64)")
    for i, t in enumerate(SEGS_MAP):
        try:
            r = fetch_region(t, 0.10)
            if r is None:
                log(f"  map[{i}] no region"); continue
            ms, _ = r
            np.save(os.path.join(OUT, f"tunedmap_{i}.npy"),
                    infer_map(model, torch, ms, 64))
            log(f"  map[{i}] {t['seg'].split('/')[-2][:22]} done")
        except Exception as e:
            log(f"  map[{i}] failed: {e}")
