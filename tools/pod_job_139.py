...
SEG = "PHerc0139/segments/20250108000000-w025_2025010863/"
SV = SEG + "surface-volumes/1.129um-0.22m-59keV-volume-20260413113053-L1.zarr/"
INK = SEG + "ink-detection/downsampled/PHerc0139-20250108000000-1.129um-0.22m-59keV-volume-20260413113053-L1-20260709123958-mrg20736-1um-s1z2-tile256-stride128-ds8.jpg"
OUT = "/workspace/out"
...
def main():
    ...
    c = s / float(w * w)
    c[c < 0.08] = np.inf
    iy, ix = np.unravel_index(int(np.abs(c - 0.12).argmin()), c.shape)
